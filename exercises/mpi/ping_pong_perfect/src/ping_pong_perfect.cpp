...
int main(int argc, char* argv[]) {
    ...
    if (process_count != 2) {
        std::cerr << "Error: Este programa requiere exactamente 2 procesos, pero se lanzó con " << process_count << " procesos." << std::endl;
        MPI_Finalize();
        return EXIT_FAILURE;
    }
    ...
    int wait_time = 0;
    if (argc == 3) {
        wait_time = std::atoi(argv[2]); // Convertir el argumento a entero
    }
