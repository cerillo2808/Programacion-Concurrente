...
/**
 @brief Estructura que contiene los datos dinámicos de cada hilo

 @param lineas Arreglo de cadenas que contiene las líneas del archivo de
 entrada, cada línea representa una plate
 @param num_lines Cantidad total de líneas (plates) leídas del archivo
 @param next_line Índice de la siguiente línea a procesar por el hilo
 @param mutex Mutex para sincronizar el acceso a las líneas entre hilos
 @param jobPath Ruta del archivo de trabajo
 @param shared_data Puntero a la estructura de datos compartidos del programa

 Esta estructura permite que cada hilo procese dinámicamente las líneas del
 archivo de entrada, asegurando que no haya conflictos al acceder a las mismas.
*/
typedef struct {
    char (*lineas)[256];
    int num_lines;
 ...
 * en una sección de la lámina asignada. Se reparte el trabajo entre
 * los hilos de forma dinámica, procesando una línea de la matriz de
